...
/*
* 
 ute the distance between a cluster center and an individual pixel.
*
...
void Slic::getResults(IplImage *image,vector <Region> &regions,vec2di &labels){
	
	//cout<<"Width: "<<image->width<<endl;
	//cout<<"Height: "<<image->height<<endl;
	//cout<<"Size: "<<regions.size()<<endl;
	//int max=-1;
	int prev=clusters[0][0],curr;
	for (int i = 0; i < image->width; i++) { 
		for (int j = 0; j < image->height; j++) {
			//regions[clusters[i][j]].addPixel(Coord(j,i));	//Change This
			//cout<<"Size: "<<regions.size()<<"\tValue: "<<clusters[i][j]<<endl;
			
			//cout<<"Value: "<<clusters[i][j]<<endl;
			
			if(clusters[i][j]==-1)	//To solve error of vector subscript
				clusters[i][j]=prev;
			prev=clusters[i][j];
			/*else{
				curr=clusters[i][j];
	...
	//cout<<"Cluster: "<<clusters[image->width-1][image->height-1]<<endl;
	//cout<<"Before: "<<regions.size()<<endl;
	
	while(regions[regions.size()-1].getNoOfPixels()==0)
		regions.erase(regions.end()-1);
	
	//cout<<"After: "<<regions.size()<<endl;
	//int size=nr_superpixels+100;
	//vector<Coord> *boundaries=new vector<Coord>[size];
	...
	for (int i = 0; i < image->width; i++) {
		for (int j = 0; j < image->height; j++) {
			...
			/* Add the pixel to the contour list if desired. */
			if (nr_p >= 1) {
				regions[clusters[i][j]].addBoundaryPixel(Coord(j,i));
				//boundaries[clusters[i][j]].push_back(Coord(i,j));
				istaken[i][j] = true;
	...
	/*for (int i = 0; i < regions.size(); i++) {
		...
		if(i%5==0)
			...
			colour=CV_RGB(128,0,200);
		else
			colour=CV_RGB(0,255,128);

		for (int j = 0; j < regions[i].getNoOfBoundaryPixels(); j++) {
			Coord o=regions[i].getBoundaryPixel(j);
			int r=o.row;
			int c=o.col;
			cvSet2D(image, r,c, colour);			
		}		
	}*/
}
